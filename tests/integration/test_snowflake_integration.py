...
class TestSnowflakeIntegration:
    ...
    def test_snowflake_specific_functions(self, use_physical_tables):
        ...
        def query_snowflake_functions():
            return TestCase(
                query="""
                    SELECT
                        1 as customer_id,
                        'Snowflake Functions Test' as customer_name,
                        COUNT(*) as order_count,
                        SUM(amount) as total_spent,
                        AVG(amount) as avg_order_value
                    FROM orders
        ...
        # Execute the test
        results = query_snowflake_functions()
        assert len(results) == 1
        assert hasattr(results[0], "total_spent")
        assert hasattr(results[0], "order_count")
    ...
    def test_decimal_precision(self, use_physical_tables):
        ...
        def query_decimal_precision():
            return TestCase(
                query="""
                    SELECT
                        'Precision Test' as category,
                        COUNT(*) as product_count,
        ...
        # Execute the test
        results = query_decimal_precision()
        assert len(results) == 1
    ...
    def test_cte_transformation(self, use_physical_tables):
        ...
        # Execute the test
        results = query_cte_transformation()
        assert len(results) >= 1
        assert all(hasattr(result, "customer_id") for result in results)
    ...
    def test_unqualified_table_names_with_default_namespace(self, use_physical_tables):
        ...
        - default_namespace='test_db.sqltesting' qualifies them to:
          'test_db.sqltesting.customers' and 'test_db.sqltesting.orders'
        ...
        # were correctly resolved to 'test_db.sqltesting.customers' and 'test_db.sqltesting.orders'
    ...
    def test_case_insensitive_table_matching(self, use_physical_tables):
        ...
        - Mock tables use lowercase database context: 'test_db.sqltesting'
        - SQL query uses mixed case table names: 'CUSTOMERS', 'orders'
        - execution_database uses lowercase: 'test_db.sqltesting'
        ...
        def query_mixed_case_tables():
            return TestCase(
                ...
                # 'CUSTOMERS' (uppercase) should match mock table 'test_db.sqltesting.customers'
                # 'orders' (lowercase) should match mock table 'test_db.sqltesting.orders'
                query="""
                    SELECT
                        c.customer_id,
                        c.name as customer_name,
        ...
        # were correctly matched with lowercase mock tables 'test_db.sqltesting.customers'
        # and 'test_db.sqltesting.orders' using case-insensitive matching
        assert len(results) == 2
        assert results[0].customer_id == 1
        assert results[0].customer_name == "Alice"
        assert results[0].order_count == 1
