"""Integration tests for DuckDB adapter with pytest configuration."""
...
class TestDuckDBIntegration:
    ...
    def test_unqualified_table_names_with_default_namespace(self, use_physical_tables):
        ...
        This test demonstrates the key role of default_namespace for DuckDB:
        - Query uses unqualified table names: 'customers' and 'orders'
        - default_namespace='main' qualifies them to:
          'main.customers' and 'main.orders'
        ...
        def query_unqualified_tables():
            return TestCase(
                ...
                # 'customers' becomes 'main.customers'
                # 'orders' becomes 'main.orders'
                default_namespace="main",
                use_physical_tables=use_physical_tables,
            )
        ...
        # were correctly resolved to 'main.customers' and 'main.orders'
    ...
    def test_duckdb_specific_array_functions(self, use_physical_tables):
        """Test DuckDB-specific array and list functionality."""
        ...
        assert len(results) >= 1
        assert all(result.customer_id in [1, 3] for result in results)

    def test_duckdb_struct_operations(self, use_physical_tables):
        """Test DuckDB struct/map operations."""
        ...
        # Execute the test
        results = query_with_structs()
        assert len(results) == 1
        assert results[0].customer_id == 1

    def test_duckdb_analytical_window_functions(self, use_physical_tables):
        """Test DuckDB advanced analytical functions."""
        ...
        def query_advanced_analytics():
            return TestCase(
                query="""
                    SELECT
                        customer_id,
                        'Analytics' as customer_name,
                        ROW_NUMBER() OVER (ORDER BY amount DESC) as order_count,
                        amount as total_spent,
                        LAG(amount, 1, 0) OVER (ORDER BY order_date) as avg_order_value
                    FROM orders
        ...
        # Execute the test
        results = query_advanced_analytics()
        assert len(results) == 1
        assert hasattr(results[0], "customer_id")
        assert results[0].order_count == 1
